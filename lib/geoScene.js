...
module.exports = function ({ renderer, camera, scene, palettes, envMap, loop }) {
  ...
  loop.on('tick', (dt) => {
    ...
    meshes.forEach((m) => {
      ...
      let meshHeight = m.boundingRegion.max.y - m.boundingRegion.min.y;
      meshHeight = Math.max(0.1, meshHeight);
      if (m.material.uniforms && m.material.uniforms.dance.value > 0) {
        meshHeight *= 2;
      }
      if (m.active &&
