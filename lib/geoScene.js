...
const INITIAL_PARTS = 0;

module.exports = function ({ renderer, camera, scene, palettes }) {
  ...
  function nextGeometry (opt = {}) {
    const additional = defined(opt.additional, ADDITIONAL_PARTS);
    for (let i = 0, count = 0; i < meshes.length && count < additional; i++) {
      const m = meshes[i];
  ...
  function resetMesh (mesh, opt = {}) {
    ...
    if (mesh.material.uniforms) {
      mesh.material.uniforms.dance.value = (opt.dance && Math.random() > 0.5) ? random(0, 1) : 0;
    }
    setRandColor(mesh);
