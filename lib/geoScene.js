...
const TOTAL_PARTS = isMobile ? 60 : 300;
const INITIAL_PARTS = 100;
...
module.exports = function ({ renderer, camera, scene, palettes, envMap, loop }) {
  ...
  // 
  const containers = newArray(5).map(() => {
    const object = new THREE.Object3D();
    const tmp = randomSphere([], 2);
    object.position.x = tmp[0];
    object.position.z = tmp[1];
    return object;
  });
  containers.forEach(c => scene.add(c));
  const meshes = [];
  ...
  loop.on('tick', (dt) => {
    ...
    meshes.forEach((m) => {
      ...
      m.container.position.y += (dt / 1000) * m.speed * api.globalSpeed;
      m.rotationY += (dt / 1000) * m.rotationFactor;
      m.quaternion.copy(camera.quaternion);
      // m.lookAt(camera.position);
      // m.rotation.x += m.rotationX;
      // m.rotation.y += m.rotationY;
      if (m.isGroup) {
      ...
      if (m.active &&
          (m.container.position.y > (meshHeight * 2 + tmpVec.y + 5) ||
          m.container.position.y < (LOWEST_Y - meshHeight * 2))) {
        m.active = false;
        m.visible = false;
      }
    });
  ...
  function resetMesh (mesh, opt = {}) {
    ...
    mesh.container.position.y = random(yOff[0], yOff[1]);
    mesh.active = true;
    mesh.visible = true;
  ...
  function addMesh (geom, mat, opt) {
    ...
    mesh.container.position.y = random(INITIAL_Y[0], INITIAL_Y[1]);
