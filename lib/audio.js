...
module.exports = function () {
  ...
  Object.defineProperty(player, 'playbackRate', {
    ...
    set: function (val) {
      playbackRate = val;
      if (playingAudio && playingAudio.element) {
        playingAudio.element.playbackRate = playbackRate;
      }
    }
  });
  ...
  function createEffectNode (output) {
    if (supportReverb) {
      ...
      reverb.time = 4.0; // seconds
      reverb.wet.value = 0;
      reverb.dry.value = 1;
      reverb.filterType = 'highpass';
      reverb.cutoff.value = 100; // Hz
  ...
  function update (dt) {
    ...
    if (!ended && Math.abs(playingAudio.currentTime - playingAudio.duration) < 0.1) {
      ended = true;
      queue();
      playQueued();
    }
  ...
  function playQueued () {
    ...
    if (queuedAudio.element) queuedAudio.element.playbackRate = playbackRate;
    ended = false;
    queuedAudio.stop();
    queuedAudio.play();
