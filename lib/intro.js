...
module.exports = function () {
  ...
  function animateOut (element, opt = {}, cb = noop) {
    ...
    children.forEach((child, i) => {
      ...
      update({ target: tween });
      tweenr.to(tween, { delay: delay, opacity: 0, duration: duration * 0.85, ease: 'quadOut' })
      const lastTween = tweenr.to(tween, { delay: delay, yOff: yOff * 0.75, duration: duration * 1, ease: 'expoOut' })
        .on('update', update);
      delay += 0.05;
